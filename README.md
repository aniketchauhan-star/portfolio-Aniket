...
| | |
| --- | --- |
| `projects` | six real, playable builds are listed. Append more entries as they are ready |
...
Six builds ship inside this site — two standalone games (`Platform Adventure`,
a canvas platformer, and `Heavy and Light`, a balance-scale game) and four
interactive 3D flipbooks, each with games embedded in its pages. Every one is a
...
public/games/heavy-and-light/
  index.html               ← the entire game, one 123KB file
  game assets/   bird/   things/   weight machine/   sounds/
```
...
`heavy-and-light` is the odd one out and the simplest: not a flipbook and not
a build with an embedded game, but a single self-contained `index.html` — the
whole thing, logic and layout, in one 123KB file with no dependencies. A bird
introduces the idea, the balance assembles itself piece by piece, and then
eight rounds ask which of two objects is heavier. The beam is a real rig rather
than a swapped picture: it pivots on its hub and carries the pans and their
contents, with pivot points and sprite anchors measured off the artwork's own
pixels. Fixed 1920×1080, scaled uniformly, always exactly 16:9.

Two of its files carry a **trailing space before the extension** — `bird/bird
happy jump .webp` and `game assets/toy elephant .webp`. That is legal, the
references match, and both serve correctly here (verified) — but it is the kind
of name that silently breaks on a case- or whitespace-normalising host, so do
not "tidy" either one without editing the asset map in `index.html` to match.

Its `README.md`, `vercel.json` and `_headers` were not copied: the first is
authoring documentation (its substance is in this section), and the last two are
standalone-host cache config that does nothing nested inside Next's `public/`.

`royal-bloom-fest`'s `asset-manifest.js` is generated by `tools/gen-asset-manifest.mjs` and
